...
## Vercel 無料枠（CPU/実行時間）を節約する

このプロジェクトは `/api/*` のサーバーレス関数（NHKニュース・週間天気など）を使うため、Vercel の無料枠の CPU 時間を消費します。

### すでにやっていること
- **ニュース**: 30分ごとの取得（`News.tsx` で `1800000`）
- **週間天気**: 1時間ごとの取得（`Clock.tsx` で `3600000`）
- **画像**: `/api/image-proxy` は使わず、NHK の画像URLを直参照（Vercel の関数実行を削減）

### さらに節約したい場合
1. **間隔をさらに伸ばす**  
   `src/components/News.tsx` の `1800000`（30分）、`src/components/Clock.tsx` の `3600000`（1時間）を大きくする（例: 1時間・2時間）。
2. **API を別サービスに移す**  
   フロントだけ Vercel、API は Cloudflare Workers・Railway・Render などの別無料枠に移すと、Vercel のサーバーレス実行をほぼゼロにできます。
