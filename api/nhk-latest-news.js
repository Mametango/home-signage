// Vercel Serverless Function: NHK新着ニュース一覧をスクレイピング
// フロントエンドからは /api/nhk-latest-news で呼び出す
...
export default async function handler(req, res) {
  ...
  try {
    const url = 'https://news.web.nhk/newsweb/pl/news-nwa-latest-nationwide';
    ...
    if (!response.ok) {
      throw new Error(`NHK latest news fetch error: ${response.status}`);
    }
