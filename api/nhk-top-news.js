// Vercel Serverless Function: NHKトップニュースをスクレイピング
// フロントエンドからは /api/nhk-top-news で呼び出す
...
export default async function handler(req, res) {
  ...
  try {
    const url = 'https://news.web.nhk/newsweb';
    ...
    if (!response.ok) {
      throw new Error(`NHK top news fetch error: ${response.status}`);
    }
    ...
    $('article').each((index, element) => {
      ...
      if (title && link && link.includes('/newsweb/') && !link.includes('/area/') && !link.includes('/genre/') && !link.includes('/pl/')) {
        if (!seenLinks.has(link) && newsItems.length < 200) {
          seenLinks.add(link);
          newsItems.push({
    ...
    // ニュースリンクからも取得
    $('a[href*="/newsweb/"][href*="/na/"], a[href*="/newsweb/"][href*="/nb/"]').each((index, element) => {
      ...
      // 日時を探す（親article内のtime/日付要素）
      let pubDate = '';
      const dateEl = parentArticle.find('time, [class*="date"], [class*="time"]').first();
      if (dateEl.length > 0) {
      ...
      if (title && link && title.length > 10 && !seenLinks.has(link)) {
        seenLinks.add(link);
        newsItems.push({
          id: newsItems.length + 1,
          title: title,
    ...
    console.error('Unexpected error in /api/nhk-top-news:', error);
